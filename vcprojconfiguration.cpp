...
// sections are divided wrt to VC++7 (rescrambled for any reason by MS from VC++6)
	...
	szCommandLine = ReplaceMacros(szCommandLine);
...
// sections are divided wrt to VC++7 (rescrambled for any reason by MS from VC++6)
	...
	szCommandLines = ReplaceMacros(szCommandLines);
...
CString vcprojconfiguration::ExpandMacros(/*in*/CString &szInputString)
{
	...
	if (!szOutputDir.IsEmpty())
	{
		...
		while ( (n=s.Find(0, "$(OutDir)"))>-1)
		{
			...
			s = s.Left(n) + 
				szOutputDir;
			tmp=s.ExcludeLeft(n+(CString("$(OutDir)").GetLength()));
			s=s+tmp;
		}
	} // end if
	...
	if (!szIntermediateDir.IsEmpty())
	{
		...
		while ( (n=s.Find(0, "$(IntDir)"))>-1)
		{
			...
			s = s.Left(n) + 
				szIntermediateDir;
			tmp=s.ExcludeLeft(n+CString("$(IntDir)").GetLength());
			s=s+tmp;
		}
	} // end if
...
CString vcprojconfiguration::ReplaceMacros(/*in*/CString &szInputString)
{
	CString s = szInputString;
	...
	while ( (n=s.Find(0, "$(ProjectDir)"))>-1)
	{
		...
		tmp=CString("$(ProjDir)");
		s=s+tmp;
		tmp=s.ExcludeLeft(n+CString("$(ProjDir)").GetLength());
		s=s+tmp;
	}
	...
	while ( (n=s.Find(0, "$(NoInherit)"))>-1)
	{
		CString tmp;
		s = s.Left(n);
		tmp=s.ExcludeLeft(n+CString("$(NoInherit)").GetLength());
		s=s+tmp;
	}
	...
	while ( (n=s.Find(0, "$(Inherit)"))>-1)
	{
		CString tmp;
		s = s.Left(n);
		tmp=s.ExcludeLeft(n+CString("$(Inherit)").GetLength());
		s=s+tmp;
	}
